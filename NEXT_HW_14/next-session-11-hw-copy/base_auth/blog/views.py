...
def detail(request, post_pk):
    ...
    if request.method == "POST":
        content = request.POST["content"]
        photo = request.FILES.get("photo")  # 이미지 파일 가져오기
        
        if photo:
            photo_name = get_new_file_name(photo, request.user, title=content)
            saved_path = default_storage.save(photo_name, photo)
            photo_url = default_storage.url(saved_path)
            comment = Comment.objects.create(post=post, content=content, creator=request.user, photo=photo)
        else:
            comment = Comment.objects.create(post=post, content=content, creator=request.user)
